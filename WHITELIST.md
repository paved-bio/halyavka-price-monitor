...
| `tutu` | Tutu.ru | `tutu.ru/poezda/…`, `tutu.ru/aviabilety/…` | Цена билетов (маршрут) |
| `aviasales` | Aviasales | `aviasales.ru/search/…` | Цена перелёта (маршрут) |
...
| Режим | Задачи | Виджет сравнения | CPA-ссылки | Биржа |
|-------|--------|------------------|------------|-------|
| **Соло** | Локально в `chrome.storage` | ❌ | ❌ | ❌ |
| **Воркер** | Облако + P2P | ✅ | ✅ | скрыта по умолчанию |
| **Premium** | Облако без P2P (за достижения) | ✅ | ✅ | скрыта по умолчанию |

Переключение: `POST /api/v1/user/tracker_mode` или кнопки в popup.

---

## Воркер vs Premium

| | **Воркер (бесплатно)** | **Premium (за достижения)** |
|--|------------------------|------------------------------|
...
| Как получить | Включить в расширении | Рефералы / топ / донат / админ — **не через оплату в приложении** |
| Первые 30 дней | Триал (добавление + воркер) | — |

После триала без воркера (и без выданного Premium) новые задачи не добавляются — включите Воркер.
...
- CPA-подмена ссылок: [`referral_rewrite.js`](referral_rewrite.js) — только worker/premium на whitelist-доменах
- Биржа (отдельно): [`category_hosts.js`](category_hosts.js) — скрыта флагом `exchange_public_enabled`
